...
class CoordinatorAgent:
    ...
    async def execute_workflow(
        ...
        trace.emit(case_id=case_id, event_type="task_assigned", actor="coordinator", target="entity_agent")
        ...
        trace.emit(case_id=case_id, event_type="handoff", actor="coordinator", target="policy_agent")
        policy = await self.policy_agent.resolve_conflict(case, entity, shipment, payment, gateway, trace)

        # --- Bước 4: Verify + Format Output ---
        trace.emit(case_id=case_id, event_type="handoff", actor="coordinator", target="verifier_agent")
        output = self.verifier_agent.verify_and_format(case, entity, shipment, payment, policy, trace)
